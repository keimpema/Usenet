...
namespace Usenet.Nzb
{
    ...
    public class NzbWriter
    {
        ...
        {
            writer.WriteStartElement(null, NzbKeywords.Head, null);
            foreach (KeyValuePair<string, ImmutableHashSet<string>> header in nzbDocument.MetaData)
            {
                foreach (string value in header.Value)
                {
                    writer.WriteStartElement(null, NzbKeywords.Meta, null);
                    writer.WriteAttributeString(null, NzbKeywords.Type, null, header.Key);
        ...
        private static void WriteFiles(XmlWriter writer, NzbDocument nzbDocument)
        {
            foreach (NzbFile file in nzbDocument.Files)
            {
                writer.WriteStartElement(null, NzbKeywords.File, null);
                writer.WriteAttributeString(null, NzbKeywords.Poster, null, file.Poster);
                writer.WriteAttributeString(null, NzbKeywords.Date, null, file.Date.ToUnixTimeSeconds().ToString(CultureInfo.InvariantCulture));
                writer.WriteAttributeString(null, NzbKeywords.Subject, null, file.Subject);
        ...
        {
            writer.WriteStartElement(null, NzbKeywords.Groups, null);
            foreach (string group in file.Groups)
            {
                writer.WriteElementString(null, NzbKeywords.Group, null, group);
            }
            writer.WriteEndElement();
        }
        ...
        {
            writer.WriteStartElement(null, NzbKeywords.Segments, null);
            foreach (NzbSegment segment in file.Segments)
            {
                writer.WriteStartElement(null, NzbKeywords.Segment, null);
                writer.WriteAttributeString(null, NzbKeywords.Bytes, null, segment.Size.ToString(CultureInfo.InvariantCulture));
                writer.WriteAttributeString(null, NzbKeywords.Number, null, segment.Number.ToString(CultureInfo.InvariantCulture));
